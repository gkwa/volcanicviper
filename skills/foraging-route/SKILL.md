...
The step is done when one decimal pair is stated back to the user together with which part of their input it came from.

## Step 2: Read the stops from the note

Every `## ` section of the note whose body contains a bare line of the form `<lat>, <lon>` is one stop, named by its heading.
...
The step is done when the stop count is stated and every stop has a heading and a decimal pair.
...
Parse the JSON with a short script run through `uv run --no-active`, never by eye.
...
Each waypoint's `distance` is how far OSRM snapped that point to the nearest path, which is worth mentioning when the start snaps more than about 50ft.

The step is done when every stop has a position in the order and a leg distance.

## Step 5: Build the links

The map link draws the walk on the OSRM demo frontend, one `loc` per point in solved order, latitude first, with `profile=2` for foot:
...
Give distances in miles, dividing metres by 1,609.344 and writing them squashed like `0.35mi`, and the total time compactly like `1h9m`.

Lead with the total distance and time, then a table of the order.
